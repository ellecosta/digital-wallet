...
describe("Transaction base validations", () => {
  ...
  it("should pass validation with valid amount", () => {
    expect(
      () => new TestTransaction(walletId, 100, TransactionType.DEPOSIT),
    ).not.toThrow();
  });

  it("should throw error if amount is zero", () => {
    expect(
      () => new TestTransaction(walletId, 0, TransactionType.DEPOSIT),
    ).toThrow("Amount must be greater than zero");
  });

  it("should throw error if amount is negative", () => {
    expect(
      () => new TestTransaction(walletId, -50, TransactionType.DEPOSIT),
    ).toThrow("Amount must be greater than zero");
  });

  it("should throw error if walletID is empty", () => {
    expect(() => new TestTransaction("", 100, TransactionType.DEPOSIT)).toThrow(
  ...
  it("should throw error if amount is NaN", () => {
    expect(
      () => new TestTransaction(walletId, NaN, TransactionType.DEPOSIT),
